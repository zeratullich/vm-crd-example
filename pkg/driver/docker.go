...
func (d *DockerDriver) CreateServer(req *CreateRequest) (*CreateReponse, error) {
	...
	resp, err := d.client.ContainerCreate(context.Background(), config, hostConfig, nil, nil, req.Name)
	if err != nil {
		return nil, err
	}
...
func (d *DockerDriver) GetServerStatus(name string) (*GetStatusResponse, error) {
...
func calculateCPUPercentUnix(previousCPU, previousSystem uint64, v *types.StatsJSON) float64 {
	var (
		...
		systemDelta = float64(v.CPUStats.SystemUsage) - float64(previousCPU)
	)
